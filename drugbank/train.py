...
from dataset import load_ddi_dataset, load_ddi_dataset_cold
from log.train_logger import TrainLogger
from model import RFSA_DDI
import argparse
from metrics import *
from utils import *
...
def main():
    ...
    # Add argument
    parser.add_argument('--n_iter', type=int, default=8, help='number of iterations')
    ...
    TOTAL_NUM_RELS = total_num_rel(name=data_set)
    hidden_dim = 128
    data_path = os.path.join(data_root, data_set)
    data_path_cold = 'data/preprocessed/cold_start/drugbank'
    ...
    model = RFSA_DDI(node_dim, edge_dim, TOTAL_NUM_RELS, dropout, hidden_dim, n_iter).cuda()
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_acc, val_auroc, val_f1_score, val_precision, val_recall, val_ap = val(model, criterion,
                                                                                            val_loader, device)
